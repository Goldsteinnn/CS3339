...
class Dissembler:
    ...
    def get_opcode(self):
        for j in range(len(binMem)):
            opcode.append(int(binMem[j], base=2) >> 21)
        ...
        for j in range(len(opcode)):
            if opcode[j] == 1112:
                ...
                instrSpaced.append(str(binMem[j][0:11]) + " " + str(binMem[j][11:16]) + " " + str(binMem[j][16:22])
                                   + " " + str(binMem[j][22:27]) + " " + str(binMem[j][27:32]))
            elif opcode[j] == 1624:
...
for i in range(len(instructions)):
    mem.append((int(instructions[i], base=2)))  # stores binary value for analysis
    binMem.append(instructions[i])  # stores full value for pretty printing
